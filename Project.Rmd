...
pairwise.chisq.test(x = d.rm$d_410_atopic,
                    g = d.rm$ages, 
                    p.adj = "bonf")
tab <- table(g, x)
pairwise.fisher.test(tab, p.adj = "bonf")

pairwise.chisq.test(x = , g, p.adj = "bonf")
...
pairwise.chisq.test(d.rm$q_410_urtic_cur[which(d.rm$q_410_urtic_cur%in%c("no","yes"))],
                    d.rm$ages[which(d.rm$q_410_urtic_cur%in%c("no","yes"))])
...
left Biphasic, top - Hypotension as a symptom. 

```{r}
table(d.rm$q_130_biphasic_v4,d.rm$q_114_hypotension_collapse_v5) %>% {.[2:3,]}
table(d.rm$q_130_biphasic_v4,d.rm$q_114_hypotension_collapse_v5) %>% {.[2:3,]} %>% summary()
```
