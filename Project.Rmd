...
```{r}
table(d.rm$q_4212_exercise_intensity,d.rm$ages)
pairwise.chisq.test(d.rm$q_4212_exercise_intensity,d.rm$ages)
```
...
```{r}
table(d.rm$q_4241_menstruation,d.rm$ages)
valF(d.rm$q_4241_menstruation)
```
...
```{r}
temp <- split(d.rm, d.rm$ages)
lapply(temp,function(age){
  table(age$q_130_biphasic_v4,age$q_114_hypotension_collapse_v5) %>% {.[2:3,]} %>% summary()
})
```
...
table(d.rm$q_130_biphasic_v4,d.rm$q_310_trigger) %>% {.[2:3,1:2]} %>% prop.table(2)
